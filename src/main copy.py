...
# Step 2: Read the data dictionary json
reference_path = files.get('files', {}).get('reference')
reference_data_excel_path = files.get('files', {}).get('reference_data_excel')
input_file_path = files.get('files', {}).get('input_file')
output_file_all = files.get('files', {}).get('output_file_all')
output_file_path_premium = files.get('files', {}).get('output_file_premium')
output_file_factor_and_premium = files.get('files', {}).get('output_file_factor_and_premium')
...
output_df = process_all_calculations(input_df, dataframes)
