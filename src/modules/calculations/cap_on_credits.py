...
def cap_on_credits_factor_calc(input_df,dataframes, input_attributes):
    ...
    mul_fact_aop = input_df[['protection_device_aop_factor','senior_discount_aop_factor','employee_discount_aop_factor','wind_loss_mitigation_aop_factor','bceg_aop_factor']].prod(axis=1).round(5)
    mul_fact_nhw = input_df[['protection_device_nhw_factor','senior_discount_nhw_factor','employee_discount_nhw_factor','wind_loss_mitigation_nhw_factor','bceg_nhw_factor']].prod(axis=1).round(5)
    mul_fact_hur = input_df[['protection_device_hur_factor','senior_discount_hur_factor','employee_discount_hur_factor','wind_loss_mitigation_hur_factor','bceg_hur_factor']].prod(axis=1).round(5)
